...
const els = {
  authBadge:$('authBadge'), settingsBtn:$('settingsBtn'), setupDialog:$('setupDialog'),
  setupForm:$('setupForm'), projectId:$('projectId'), clientId:$('clientId'), mapsKey:$('mapsKey'),
  clearConfigBtn:$('clearConfigBtn'), runSelect:$('runSelect'), fhRange:$('fhRange'), fhNumber:$('fhNumber'),
  validTime:$('validTime'), statSelect:$('statSelect'), refreshBtn:$('refreshBtn'), homeBtn:$('homeBtn'),
  precipControls:$('precipControls'), probControls:$('probControls'), qpfThreshold:$('qpfThreshold'),
  opacityRange:$('opacityRange'), loading:$('loading'), message:$('message'), messageSetupBtn:$('messageSetupBtn'),
  legend:$('legend'), legendTitle:$('legendTitle'), legendGradient:$('legendGradient'),
...
function showMessage(title, body, showSetup=true){
  ...
  const b=$('dynamicSetup'); if(b)b.onclick=openSetup;
}
...
function initMap(){
  ...
  map=new google.maps.Map($('map'),{
    center:LIX_CENTER,zoom:LIX_ZOOM,mapTypeId:'terrain',
...
function authenticate(){
  ...
  loadGoogleMaps(cfg.mapsKey).then(()=>{
    ...
    ee.data.authenticateViaOauth(
      ...
      ()=>{setLoading(false);showMessage('Sign in required','Use Setup → Save & connect, then sign in with the Google account approved for WeatherNext.',true)},
      true
    );
...
function connectionError(err){
  ...
  showMessage('Connection failed',escapeHtml(msg)+'<br><br>Verify the Cloud project, OAuth origin, Earth Engine API, and that you signed in with the allowlisted WeatherNext account.',true);
}

function loadRuns(){
  setLoading(true,'Finding recent WeatherNext runs…');
  const now=ee.Date(Date.now());
  const recent=ee.ImageCollection(DATASET).filterDate(now.advance(-48,'hour'),now.advance(1,'hour'));
  ee.List(recent.aggregate_array('start_time')).distinct().sort().reverse().slice(0,30).evaluate((runs,err)=>{
    setLoading(false);
    if(err||!runs?.length){connectionError(err||new Error('No recent model runs returned.'));return}
...
function formatRun(iso){
  ...
  return d.toISOString().slice(0,10)+' '+String(d.getUTCHours()).padStart(2,'0')+'Z';
}
function updateHorizon(){
  const d=new Date(els.runSelect.value);
  const synoptic=[0,6,12,18].includes(d.getUTCHours());
  const max=synoptic?360:48;
...
function updateValidTime(){
  const run=new Date(els.runSelect.value);const fh=+els.fhRange.value;
...
function baseRunCollection(){
  return ee.ImageCollection(DATASET).filter(ee.Filter.eq('start_time',els.runSelect.value));
}
function getSingleImage(stat){
  const fh=+els.fhRange.value;
  return ee.Image(baseRunCollection().filter(ee.Filter.eq('forecast_hour',fh)).first());
}
function buildStandardImage(){
  ...
  const img=getSingleImage(stat).select(p.band+'_'+stat);
  return p.transform(img).rename('value');
}
function buildProbabilityRangeImage(){
  const thresholdIn=parseFloat(els.qpfThreshold.value)||0.25;
  const t=thresholdIn/39.3700787;
  const img=getSingleImage('mean');
  ...
  // Representative values visualize rigorous probability brackets from the published quantiles:
  // >90, 75-90, 50-75, 25-50, 10-25, <10 percent.
  return ee.Image.constant(5)
    .where(q90.gt(t),17.5)
    .where(q75.gt(t),37.5)
    .where(q50.gt(t),62.5)
    .where(q25.gt(t),82.5)
...
function renderLayer(){
  ...
  if(currentProduct==='precip'&&accumHours>+els.fhRange.value){
    accumHours=Math.max(1,+els.fhRange.value);
  }
  currentImage=currentProduct==='qpfprob'?buildProbabilityRangeImage():buildStandardImage();
  const p=PRODUCTS[currentProduct];
  const vis={min:p.min,max:p.max,palette:palettes[currentProduct]};
  currentImage.getMap(vis,(mapId,err)=>{
    ...
    overlay.setOpacity(parseFloat(els.opacityRange.value));
...
function samplePoint(lat,lng){
  ...
  els.pointReadout.classList.remove('hidden');
  els.readoutLocation.textContent=lat.toFixed(3)+', '+lng.toFixed(3);
  els.readoutValue.textContent='…';
  currentImage.reduceRegion({reducer:ee.Reducer.first(),geometry:ee.Geometry.Point([lng,lat]),scale:11000,bestEffort:true,maxPixels:1e6})
    .get('value').evaluate((v,err)=>{
      ...
      if(currentProduct==='qpfprob'){
        const label=v>=90?'>90%':v>=75?'75–90%':v>=50?'50–75%':v>=25?'25–50%':v>=10?'10–25%':'<10%';
        els.readoutValue.textContent=label;
      }else{
        const p=PRODUCTS[currentProduct];
        const digits=currentProduct==='precip'?2:(currentProduct==='mslp'?1:0);
...
function selectProduct(name){
  currentProduct=name;
  document.querySelectorAll('.product').forEach(b=>b.classList.toggle('active',b.dataset.product===name));
  els.precipControls.classList.toggle('hidden',name!=='precip');
  els.probControls.classList.toggle('hidden',name!=='qpfprob');
...
els.runSelect.onchange=()=>{updateHorizon();renderLayer()};
els.fhRange.oninput=()=>{els.fhNumber.value=els.fhRange.value;updateValidTime()};
...
document.querySelectorAll('#accumGroup button').forEach(b=>b.onclick=()=>{
  document.querySelectorAll('#accumGroup button').forEach(x=>x.classList.remove('active'));b.classList.add('active');
  accumHours=+b.dataset.hours;
  if(accumHours>1){els.statSelect.value='mean';els.statSelect.disabled=true}else els.statSelect.disabled=!connected;
  renderLayer();
});
